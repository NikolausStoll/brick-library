...
import { KIDS_API, createKidsSet } from './helpers.mjs';

describe('kids scrape API', () => {
  let server;
  let setId;
  let pngBuffer;
  ...
  it('POST /api/kids/sets/:setId/images/scrape requires rawHtml or pageUrl + containerSelector', async () => {
    const { response, data } = await jsonRequest(
      server.baseUrl,
      `${KIDS_API}/sets/${setId}/images/scrape`,
      {
        method: 'POST',
  ...
  it('POST /api/kids/sets/:setId/images/scrape returns empty result when no images found', async () => {
    const { response, data } = await jsonRequest(
      server.baseUrl,
      `${KIDS_API}/sets/${setId}/images/scrape`,
      {
  ...
  it('POST /api/kids/sets/:setId/images/scrape parses rawHtml and downloads images', async () => {
    ...
    try {
      ...
      expect(data.images[0].url).toContain('/uploads/kids/');
    } finally {
      await mockServer.close();
    }
  });

  it('POST /api/kids/sets/:setId/images/scrape skips duplicate originalUrl values', async () => {
    const mockServer = await createMockImageServer(pngBuffer);
    try {
      const payload = {
        method: 'POST',
      ...
      const first = await jsonRequest(server.baseUrl, `${KIDS_API}/sets/${setId}/images/scrape`, payload);
      const second = await jsonRequest(server.baseUrl, `${KIDS_API}/sets/${setId}/images/scrape`, payload);
  ...
  it('POST /api/kids/sets/:setId/images/scrape normalizes containerSelector without leading dot', async () => {
    const mockServer = await createMockImageServer(pngBuffer);
  ...
  it('POST /api/kids/sets/:setId/images/scrape returns 400 when selector matches nothing', async () => {
